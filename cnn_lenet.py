...
def pooling_layer_forward(input, layer):
  ...
  for n in range(batch_size):
    input_n['data'] = input['data'][:, n]
    print (input_n['data'].shape)
    col_n = im2col_conv(input_n, layer, h_out, w_out)
    print (col_n)
    col_n_h = col_n.shape[0]
    col_n_w = col_n.shape[1]
    col_n = np.reshape(col_n, (col_n_h/c, col_n_w*c))
    print (col_n)
    tempoutput = np.max(col_n, axis=0) 
    print tempoutput.shape
    output['data'][:,n] = tempoutput
  # np.reshape(output['data'], (h_out*w_out*c, batch_size))
  # implementation ends
...
def pooling_layer_backward(output, input, layer):
  ...
  for n in range(batch_size):
    ...
    col = im2col_conv(input_n, layer, h_out, w_out)  #UNPACK
    
    col[col != np.max(col, axis=0)]=0
    col[col == np.max(col, axis=0)]=1 
    col = col * output['diff'][:,n]
    
    tmpout = col2im_conv(col, input, layer, h_out, w_out).flatten()  #REPACK
...
def relu_forward(input, layer):       #checked
  """RELU foward
  ...
  # implementation begins
  new_data = copy.deepcopy(input['data'])
...
def relu_backward(output, input, layer):      #checked
  """RELU backward
  ...
  # implementation begins
  print output
  input_od_data = output['data']     #checked
  input_od_data[input_od_data>0]=1   #derivative is one when the entry was positive
  print (output['diff']*input_od_data)
  input_od = output['diff']*input_od_data  #TO BE CHECKED
  # implementation ends
  assert np.all(input['data'].shape == input_od.shape), 'input_od has incorrect shape!'
...
def inner_product_forward(input, layer, param):    #checked
  ...
  for n in range(batch_size):
    tempoutput = np.dot(input['data'][:,n],param['w']) + param['b']    #MATRIX OPERATIONS
    output['data'][:, n] = tempoutput
  ###
  # implementation ends
...
def inner_product_backward(output, input, layer, param):    #checked
  ...
  # implementation begins
  dimension = len(param['b'])
  dL_dh = output['diff']
  #full_xs = np.array([input['data']] * dimension).T
  #print full_xs
  output_diff_1 = copy.deepcopy(output['diff'])
  output_diff_2 = copy.deepcopy(output['diff'])
  output_diff_3 = copy.deepcopy(output['diff'])
  output_diff_4 = copy.deepcopy(output['diff'])
  r = input['data'][:,0]
  r = np.reshape(r, (1, r.shape[0]))
  param_grad_w = np.repeat(r, dimension, axis=0).T #dh/dw
  param_grad['w'] = param_grad_w*output_diff_1.flatten()
  param_grad['w'] = param_grad['w'].reshape(param['w'].shape)

  #param_grad['b'] = np.ones(param['b'].shape)    # NO MULT CONSTANTS
  param_grad['b'] = output_diff_3   # NO MULT CONSTANTS
  param_grad['b'] = param_grad['b'].reshape(param['b'].shape)
    
  w_diff = param['w'] * output_diff_1.flatten()
  input_od = np.sum(np.array(w_diff), axis=1)
  input_od = np.reshape(input_od, input['data'].shape)
  
  # implementation ends

  assert np.all(input['data'].shape == input_od.shape), 'input_od has incorrect shape!'    #input data as param w
...
def sgd_momentum(w_rate, b_rate, mu, decay, params, param_winc, param_grad):
  ...
  # implementation begins
  for key in params.keys():  #get key
    a_w = w_rate
    a_b = b_rate
    param_winc_[key]['w'] = mu*param_winc_[key]['w'] + a_w*(param_grad[key]['w']+decay*params_[key]['w'])   #reg
    param_winc_[key]['b'] = mu*param_winc_[key]['b'] + a_b*param_grad[key]['b']   #reg
    params_[key]['w'] = params_[key]['w'] - param_winc_[key]['w']
    params_[key]['b'] = params_[key]['b'] - param_winc_[key]['b']
  params = copy.deepcopy(params_)
  param_winc = copy.deepcopy(param_winc_)
